...
class NewsController extends Controller
{
    ...
    */
    ...
    public function getNews($query, $offset, $market = 'en-US'){
        ...
        $response = $client->request('GET','https://api.datamarket.azure.com/Bing/Search/v1/News?Query=%27'.$q.'%27&$top=1&$skip='.$offset.'&Market=%27'.$market.'%27&$format=json&NewsSortBy=%27date%27', ['headers' => ['Authorization' =>  'Basic '.base64_encode(env('BING_SEARCH_ACCOUNT_NAME').':'. env('BING_SEARCH_KEY'))]]);
        ...
        $parsed['source'] = $news['Source']; 
        $parsed['link'] = $news['Url'];
        //Call to Alchemy to get the full body and the emotions
        $results = $this->getEmotion($news['Url']);
        if(!empty($results['body'])){
            $parsed['body'] = $this->truncate($results['body'], 400);
        } else {
